...
class _MyRequestListState extends State<MyRequestList> {
  ...
  Future<dynamic> fetchRequests() async {
    ...
    return http
        .get("http://192.168.1.100:8000/api/request/" + userId)  
        .then((response) {
  ...
  deleteRequest(String id) {
    http.delete("http://192.168.1.100:8000/api/request/" + id).then((response) {
      print(response);
    }).catchError((e) {
      print(e);
    });
      ...
      confirmDismiss: (DismissDirection direction) async {
        final bool res = await showDialog(
          ...
          builder: (context) {
            return AlertDialog(
              title: Text("Confirm"),
              content: Text("Do you wish to delete this request?"),
              actions: <Widget>[
                FlatButton(
                  child: Text("delete"),
